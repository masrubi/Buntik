                            ...
                            <form action="{{ route('pesanan.store') }}" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="col-lg-4 col-sm-6">
                                            <div class="card border rounded shipping-address">
                                                <div class="card-body">
                                                    ...
                                                    <p class="mb-1">
                                                        {{ $alamat->alamat . ', ' . $alamat->nama_desa . ', ' . $alamat->nama_kecamatan . ', ' . $alamat->nama_kabupaten . ' [' . $alamat->nama_prov . ']' }}
                                                    </p>
                                ...
                                <div class="row">
                                    @if ($alamat->count() > 0)
                                        ...
                                        <div class="mb-3">
                                            <div class="col-md-12">
                                                <label class="form-label">Pilih Aplikasi Semai Produk / <span
                                                        class="text-danger">*
                                                        Kosongkan Kalau Tidak Ada Aplikasi
                                                        Semai</span></label>
                                                <select class="form-control select2" id="semaiId">
                                                    <option value="">Pilih Aplikasi semai [Bisa Di Pilih Lebih Dari
                                                        Satu]</option>
                                                    @foreach ($semai as $semai)
                                                        <option data-doj="{{ $semai->harga_semai }}"
                                                            data-kabupaten="{{ $semai->jenis_semai }}">
                                                            {{ Str::upper($semai->jenis_semai) }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            <div class="col-md-12">
                                                <input class="form-control" type="text" name="semai"
                                                    id="semai_result" readonly>
                                                <input type="text" value="" name="semai_harga" id="semai_harga"
                                                    size="20" hidden>
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            <div class="col-md-12">
                                                <label class="form-label">Catatan Untuk Aplikasi Semai dan Variasi Produk <span class="text-danger">*
                                                    Kosongkan Kalau Tidak Ada Catatan</span></label>
                                                <div>
                                                    <textarea class="form-control" name="note" rows="5" placeholder="Catatan Variasi dan Semai Produk"></textarea>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="mb-3">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#variasiId").change(function() {
                ...
                var Item = cntrol.find(':selected').data('kabupaten');
            ...
            $("#semaiId").change(function() {
                var cntrol = $(this);

                var Item = cntrol.find(':selected').data('kabupaten');

                var variasi = $('#semai_result');
                var value = variasi.val();
                var finalvalue = value + Item + ', ';

                if (cntrol.val() == "")
                    finalvalue = "";
                $('#semai_result').val(finalvalue);
            });
            ...
            $("#semaiId").change(function() {
                ...
                var variasi_harga = $('#semai_harga').val();
                var finalharga = variasi_harga + item_harga + ', ';

                if (cntrol.val() == "")
                    finalharga = "";
                $('#semai_harga').val(finalharga);
            });
